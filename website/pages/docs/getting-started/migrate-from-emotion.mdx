...
```js
/** @jsx jsx */
import { jsx } from '@agriffis/xstyled-emotion'
...
`@agriffis/xstyled-emotion` is a replacement of `@emotion/react` and `@emotion/styled`. You can safely replace `@emotion/react` and `@emotion/styled` by `@agriffis/xstyled-emotion`.
...
JSX Pragma is exposed in `@agriffis/xstyled-emotion`:
...
To avoid the `/** @jsx jsx */` you can use `@agriffis/xstyled-babel-preset-emotion-css-prop`. It has the same interface as `@emotion/babel-preset-css-prop`, see [Emotion documentation](https://emotion.sh/docs/css-prop#babel-preset).

**.babelrc**

```js
{
  "presets": ["@agriffis/xstyled-babel-preset-emotion-css-prop"]
}
```
...
```js
{
  ...
  "presets": [
    [
      "@agriffis/xstyled-babel-preset-emotion-css-prop",
      {
...
`css` prop with `@agriffis/xstyled-emotion` is slightly different from `css` prop with `@emotion/react`.
